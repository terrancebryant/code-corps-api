defmodule CodeCorpsWeb.TaskController do
  ...
  alias CodeCorps.{Task, Policy, Project, User}
  ...
  def index(%Conn{} = conn, %{} = params) do
    with tasks <- Task |> Project.TaskQuery.filter(params) |> Ecto.Query.order_by([asc: :order]) |> Repo.all do
  ...
  def show(%Conn{} = conn, %{} = params) do
    with %Task{} = task <- Task |> Project.TaskQuery.query(params) |> Repo.one do
  ...
  def create(%Conn{} = conn, %{} = params) do
    with %User{} = current_user <- conn |> Guardian.Plug.current_resource,
         ...
         {:ok, %Task{} = task} <- params |> Project.Task.create do
  ...
  def update(%Conn{} = conn, %{} = params) do
    with %Task{} = task <- Task |> Project.TaskQuery.query(params) |> Repo.one,
         %User{} = current_user <- conn |> Guardian.Plug.current_resource,
         {:ok, :authorized} <- current_user |> Policy.authorize(:update, task),
         {:ok, %Task{} = task} <- task |> Project.Task.update(params) do
      conn |> render("show.json-api", data: task)
    end
  end
end
